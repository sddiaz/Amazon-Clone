...
import WishlistDisplay from "../components/Shared/WishlistDisplay";
...
export default function Cart() {
  ...
  >([]);
  const [wishlistData, setWishlistData] = useState<{ productData: Product }[]>(
    []
  );
  ...
  const handleFetchWishlistData = async () => {
    try {
      const result = await StoreService.getWishlistProducts(
        firebaseUserData?.userData?.wishlist
      );
      if (result) {
        console.log(result);
  ...
  return (
    <div className="font-ember flex flex-col items-center min-h-screen w-full bg-[var(--amazonGrey)] gap-y-4">
      {cartData && (
        <div className="w-full max-w-[1500px] flex flex-col md:flex-row gap-4 mt-2">
          {/* Checkout Section */}
          {cartData?.length > 0 && (
          ...
          <div className="flex flex-col gap-y-4">
            {/* Cart Items Section */}
            {cartData?.length == 0 && (
              <div className="flex-1 flex h-[100vh] justify-between bg-white p-4 order-2 md:order-1 select-none">
                <div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="w-[100px] flex font-bold text-sm font-ember justify-between items-center font-ember text-sm border-4 rounded-3xl border-[var(--amazonButton)]">
                          ...
                          <IconButton
                            size="small"
                            onClick={() =>
                              handleAddToCart(item?.productData?.id)
                            }
                          >
                            <PlusIcon />
                          </IconButton>
            ...
            {wishlistData && (
              <div className="order-1 w-full max-w-[1500px] flex flex-col md:flex-row gap-4">
                {wishlistData?.length > 0 && (
                  <div className="flex-1 bg-white p-4 shadow-md order-2 md:order-1">
                    <h2 className="text-3xl font-emberThin mb-4">Your Wishlist</h2>
                    <div className="space-y-4">
                      <WishlistDisplay
                        wishlistData={wishlistData.map(
